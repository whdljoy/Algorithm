...
def move():
	...
	for item in m_info:
		for target in m_info[item]:
			...
			# y,x, next_dir = get_next_pos(target_y, target_x, target_d)
			# tmp[(y,x)].append(next_dir)
			while (1):
				...
				if 0 <= change_y < 4 and 0 <= change_x < 4 and  ((change_y,change_x) != (pack_y,pack_x)) \
					...
					if target_d == target:
						tmp[(target_y, target_x)].append(target_d) #이전 위치 그대로 넣어줌
						print("target", target_y, target_x)
						print(target)
						break
	for change in tmp:
				...
				if 0 <= x2 < 4 and 0 <= y2 < 4 :
					for a in range(4):
						...
						if 0 <= x3 < 4 and 0 <= y3 < 4 :
							...
							if grid[y2][x2] != 0 and (y2,x2) not in visited:
								m_num += grid[y2][x2]
								visited.append((y2, x2))
							if grid[y3][x3] != 0 and (y3,x3) not in visited:
								m_num += grid[y3][x3]
							if best[0] < m_num:
								best = (m_num,i,j,a)
