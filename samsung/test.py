...
def simulate():
	...
	for i in range(n):
		for j in range(n):
			for k in range(DIR_NUM):
				if (monster[t_num-1][i][j][k]>0):
					print("check",(i,j,k),monster[t_num-1][i][j][k])
	print(px,py)
	# 각 칸에 있는 몬스터를 이동시킵니다.
	move_m()
	for i in range(n):
		for j in range(n):
			for k in range(DIR_NUM):
				if (monster[t_num][i][j][k]>0):
					print("avl",(i,j,k),monster[t_num][i][j][k])
	print("change turn",t_num)
	# 팩맨을 이동시킵니다.
	move_p()
	# for i in range(n):
	# 	for j in range(n):
	# 		for k in range(DIR_NUM):
	# 			if (monster[t_num][i][j][k]>0):
	# 				print("avl",(i,j),monster[t_num][i][j][k])
	# print("change turn",t_num)
	# 시체들이 썩어갑니다.
	decay_m()
...
def count_monster():
	...
	for i in range(n):
		for j in range(n):
			for k in range(DIR_NUM):
				cnt += monster[t][i][j][k]
				# if (monster[t][i][j][k]>0):
				# 	print("avl",(i,j),monster[t][i][j][k])
	return cnt
...
# 		for j in range(n):
# 			for k in range(DIR_NUM):
# 				if (monster[a][i][j][k]>0):
# 					print("avl",(i,j),monster[a][i][j][k])
# 	print("change turn")
print(count_monster())